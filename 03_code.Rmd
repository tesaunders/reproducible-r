...
plot_species <- function(data, x_var, y_var, colour, species) {
  ...
  # Convert variable names to strings for plot titles
  x_name <- as_label(enquo(x_var))
  y_name <- as_label(enquo(y_var))
  
  # Check the species ID exists in the data
  if (!(species %in% data$species_id)) {
    stop(paste0("Error: Species ID '", species, "' not found in the dataset."))
  }

  # Embrace variables to look in the dataframe for that column
  plot <- 
    data |> 
    filter(species_id == .env$species) |> # Look for species in the function's environment
    ggplot(aes(x = {{ x_var }}, y = {{ y_var }}, colour = {{ colour }})) +
    geom_point(alpha = 0.6) +
    theme_minimal() +
    labs(title = paste(y_name, "vs.", x_name, "for Species ID:", species))
  
  save_path <- paste0("figures/", species, "_", x_name, "_vs_", y_name, ".png")
  
  ggsave(save_path, plot = plot, width = 10, height = 6)
}

# purrr::walk iterates over valid_species to produce the plots
