...
We have done some preliminary inspecting of our data. Now let's go one step further and liken this to a more realistic workflow. Perhaps we want to look more closely at the plot type. 
...
Given that plot types concern excluding other rodents, one question we may want to answer is how this has impacted on the development of the mammals we are sampling, specifically their hindfoot_length.

As with any questions like this, a good place to start would be to visualise the data. Because we have a categorical independent variable (plot_type), let's use a boxplot.
...
It looks like there may be a relationship between hindfoot length and plot type. We can see that, for example, there looks to be quite a difference in weight between the control and the long-term krat exclosure plot-types.

Because we have a continuous dependent variable (hindfoot length) and an categorical independent variable (plot type), with multiple levels, analysis of variance (ANOVA) would be an appropriate statistical test. We can then follow up with some post-hoc comparisons to test the individual differences in weight across each plot type.

Before we do that though, there are a number of assumptions we must check:

1. Normality

As with any general linear model, we must check for normality. Here, we are looking at normality of residuals. In truth, because our sample size per group is so high, we usually wouldn't bother inspecting this, but for the sake of completeness, we will here.

There are two methods for testing this assumption, graphical and statistical. We recommend graphical methods, which include plotting the residuals in a histogram or QQ-plot. The statistical methods include normality tests such as Shapiro-Wilk, which can be rather conservative, especially given large sample sizes. 
...
We can see that the residuals do not exactly follow a normal distribution. In this case, we would be ok with this as combined with our large sample size our ANOVA will be fairly robust to any violation of normality.

2. Homogeneity - Equality of Variances

Our second assumption to check is whether the variance in hindfoot length is roughly equal across all plot types. We can do this, again, using statistical methods, graphical methods, or both. Just like normality, we will opt for a graphical method. In particular, a boxplot.
...
ggplot(surveys, aes(x = plot_type, y = hindfoot_length)) +
  ...
  coord_flip() # Using coord flip to rotate the plot so the labels are easily readable
```
It seems quite clear that this assumption has been violated. Having removed the outliers, we can see that the whiskers of the Control and Long-term Krat Exclosure plots are much smaller than the other three plot types. If the groups had similar sample sizes we would not be too worried, but they do not.
...
We can see from our pairwise tests that all of the comparisons are significant.